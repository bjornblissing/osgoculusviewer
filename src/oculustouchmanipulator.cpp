...
void OculusTouchManipulator::setHomePosition(const osg::Vec3d & eye, const osg::Vec3d & center, const osg::Vec3d & up, bool autoComputeHomePosition)
{
...
void OculusTouchManipulator::setTransformation(const osg::Vec3d & eye, const osg::Vec3d & center, const osg::Vec3d & up)
{
...
bool OculusTouchManipulator::handle(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter&)
{
	...
	if (m_device->touchControllerAvailable()) handleTouchInput();
	return true;
}
...
void OculusTouchManipulator::handleTouchInput()
{
	...
	if (thumbstickLeft.length() > 0.1) {
		...
		osg::Quat rotateYaw(-rotationScale*thumbstickLeft.x()/osg::PI_2,  m_rotation * osg::Vec3d(0.0, 1.0, 0.0));
